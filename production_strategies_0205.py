    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if not self.closed:
            return {
                ...
                # keep schema stable even with no closed trades
                "avg_hold_secs": 0.0,
                "avg_win": 0.0,
                "avg_loss": 0.0,
...
class ComprehensiveLogger:
    ...
    def _init_snapshots(self):
        ...
        with open(self.snapshots_path, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
    ...
    def _init_positions(self):
        ...
        with open(self.positions_path, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
    ...
    def log_snapshot(
        ...
        with open(self.snapshots_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
    ...
    def log_position(self, pos: ClosedPosition):
        ...
        with open(self.positions_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([
...
class GameRunner:
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        for strategy in self.strategies:
            ...
            print_status(f"  Win Rate: {stats.get('win_rate', 0.0):.1%}")
            print_status(f"  Locks: {stats.get('locks', 0)} ({stats.get('lock_rate', 0.0):.1%})")
            print_status(f"  Stops: {stats.get('stops', 0)}")
            print_status(
                f"  Gross P&L: {stats.get('gross_pnl', 0.0):.1f}¢ "
                f"(${stats.get('gross_pnl', 0.0)/100:.2f})"
            )
            print_status(
                f"  Fees: {stats.get('fees', 0.0):.1f}¢ "
                f"(${stats.get('fees', 0.0)/100:.2f})"
            )
            print_status(
                f"  Net P&L: {stats.get('net_pnl', 0.0):.1f}¢ "
                f"(${stats.get('net_pnl', 0.0)/100:.2f})"
            )
